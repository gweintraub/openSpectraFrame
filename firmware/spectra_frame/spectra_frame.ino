...
void setup() {
  ...
  // Sleep immediately if still no update needed
  if (!needsUpdate) {
...
bool fetchAndDisplayImage(float batteryVoltage, int state) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (time(NULL) < 1704067200) {
      Serial.println("FATAL ERROR: Clock is invalid. Aborting connection to maintain TLS integrity.");
      return false;
    }
